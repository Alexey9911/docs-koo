---
title: "API Introduction"
description: "Getting started with the Pixie API"
icon: "book"
---

The Pixie API provides programmatic access to intent parsing, transaction building, and portfolio tracking. Built on REST principles with JSON request/response format.

## Base URL

```
https://api.neyrs.cloud/v1
```

## Authentication

All API requests require authentication via Bearer token:

```bash
Authorization: Bearer pixie_live_abc123def456
```

## Request Format

```bash
curl --request POST \
  --url https://api.neyrs.cloud/v1/sessions \
  --header 'Authorization: Bearer pixie_live_abc123def456' \
  --header 'Content-Type: application/json' \
  --data '{
    "walletAddress": "7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU"
  }'
```

## Response Format

All responses follow a consistent structure:

```json
{
  "data": {},
  "meta": {
    "requestId": "req_abc123def456",
    "timestamp": "2024-03-08T14:30:00Z"
  }
}
```

## Error Responses

```json
{
  "error": {
    "code": "validation_error",
    "message": "Invalid request parameters",
    "details": {
      "field": "walletAddress",
      "reason": "Invalid Solana address format"
    }
  },
  "meta": {
    "requestId": "req_abc123def456",
    "timestamp": "2024-03-08T14:30:00Z"
  }
}
```

## Rate Limits

| Tier | Requests/min | Transactions/day |
|------|-------------|------------------|
| Free | 30 | 100 |
| Pro | 120 | 1,000 |
| Enterprise | 600 | Unlimited |

Rate limit headers included in every response:

```
X-RateLimit-Limit: 120
X-RateLimit-Remaining: 87
X-RateLimit-Reset: 1709913600
```

## Pagination

List endpoints support cursor-based pagination:

```bash
GET /v1/messages?limit=50&cursor=msg_abc123def456
```

Response includes pagination metadata:

```json
{
  "data": [],
  "pagination": {
    "hasMore": true,
    "nextCursor": "msg_xyz789ghi012"
  }
}
```

## Versioning

The API is versioned via URL path. Current version: v1

Breaking changes will be released as new versions (v2, v3, etc.)

## SDKs

Official SDKs available:

- TypeScript/JavaScript: `npm install @pixie-agent/sdk`
- Python: `pip install pixie-sdk`

<Note>
  API keys can be generated from the Developer Dashboard. Store keys securely and never expose them in client-side code.
</Note>
